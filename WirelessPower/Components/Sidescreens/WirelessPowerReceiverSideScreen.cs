...
{
    public sealed class WirelessPowerReceiverSideScreen : SideScreenContent
    {
        ...
        private GameObject InternalBatteryThresholdSlider;
        private GameObject InternalBatteryThresholdText;
        private WirelessPowerReceiver target;

        internal WirelessPowerReceiverSideScreen() => this.target = (WirelessPowerReceiver)null;

        public override void ClearTarget() => this.target = (WirelessPowerReceiver)null;

        public override string GetTitle() => "Receiver Configuration";

        public override bool IsValidForTarget(GameObject target) => target.GetComponentSafe<WirelessPowerReceiver>() != null;
        ...
        protected override void OnPrefabInit()
        {
            ...
            this.SetupInternalThresholdSlider(backColour, rectOffset);
        ...
        private void SetupWirelessThresholdSlider(Color backColour, RectOffset rectOffset)
		{
            ...
            thresholdSliderMin_label.Text = Mathf.RoundToInt(0).ToString();
            ...
            thresholdSliderMax_label.Text = Mathf.RoundToInt(99).ToString();
        ...
        private void SetupInternalThresholdSlider(Color backColour, RectOffset rectOffset)
        {
            PPanel internalThresholdTitle_panel = new PPanel("InternalThresholdTitleRow");
            internalThresholdTitle_panel.BackColor = backColour;
            internalThresholdTitle_panel.Alignment = TextAnchor.MiddleCenter;
            internalThresholdTitle_panel.Direction = PanelDirection.Horizontal;
            internalThresholdTitle_panel.Spacing = 10;
            internalThresholdTitle_panel.Margin = rectOffset;
            internalThresholdTitle_panel.FlexSize = Vector2.right;
            PLabel internalThresholdTitle_label = new PLabel("InternalThresholdTitleLabel");
            internalThresholdTitle_label.TextAlignment = TextAnchor.MiddleRight;
            internalThresholdTitle_label.Text = WirelessPowerStrings.InternalBatteryThresholdTitleName;
            internalThresholdTitle_label.ToolTip = WirelessPowerStrings.InternalBatteryThresholdTitleTooltip;
            internalThresholdTitle_label.TextStyle = PUITuning.Fonts.TextDarkStyle;
            PTextField internalThresholdTitle_textField = new PTextField("InternalThresholdSliderTextField")
            {
                Text = this.target.BatteryRefillPercent.ToString("0"),
                MaxLength = 3,
            };
            internalThresholdTitle_textField.ToolTip = WirelessPowerStrings.InternalBatteryThresholdTitleTooltip;
            internalThresholdTitle_textField.OnTextChanged = this.ChangeTextFieldInternalThreshold;
            internalThresholdTitle_textField.OnRealize += (PUIDelegates.OnRealize)(obj => this.InternalBatteryThresholdText = obj);
            PLabel internalThresholdUnit_label = new PLabel("InternalThresholdUnitLabel");
            internalThresholdUnit_label.TextAlignment = TextAnchor.MiddleRight;
            internalThresholdUnit_label.Text = "%";
            internalThresholdUnit_label.ToolTip = WirelessPowerStrings.InternalBatteryThresholdTitleTooltip;
            internalThresholdUnit_label.TextStyle = PUITuning.Fonts.TextDarkStyle;

            PPanel internalThresholdTitle_components = internalThresholdTitle_panel.AddChild((IUIComponent)internalThresholdTitle_label);
            internalThresholdTitle_components = internalThresholdTitle_panel.AddChild((IUIComponent)internalThresholdTitle_textField);
            internalThresholdTitle_components = internalThresholdTitle_panel.AddChild((IUIComponent)internalThresholdUnit_label);
            internalThresholdTitle_components.AddTo(this.gameObject, -2);

            PPanel internalThresholdSlider_panel = new PPanel("InternalThresholdSliderRow");
            internalThresholdSlider_panel.BackColor = backColour;
            internalThresholdSlider_panel.ImageMode = Image.Type.Sliced;
            internalThresholdSlider_panel.Alignment = TextAnchor.MiddleCenter;
            internalThresholdSlider_panel.Direction = PanelDirection.Horizontal;
            internalThresholdSlider_panel.Spacing = 16;
            internalThresholdSlider_panel.Margin = new RectOffset(12, 12, 6, 32);
            internalThresholdSlider_panel.FlexSize = Vector2.right;

            PLabel internalThresholdSliderMin_label = new PLabel("InternalThresholdSliderMinLabel");
            internalThresholdSliderMin_label.Text = Mathf.RoundToInt(1).ToString();
            internalThresholdSliderMin_label.TextStyle = PUITuning.Fonts.TextDarkStyle;

            PPanel internalThresholdSlider_components = internalThresholdSlider_panel.AddChild((IUIComponent)internalThresholdSliderMin_label);

            PSliderSingle internalThresholdSlider = new PSliderSingle("InternalThreshold")
            {
                Direction = Slider.Direction.LeftToRight,
                HandleColor = PUITuning.Colors.ButtonPinkStyle,
                HandleSize = 20.0f,
                InitialValue = this.target.BatteryRefillPercent,
                IntegersOnly = true,
                MaxValue = 100,
                MinValue = 1,
                PreferredLength = 140.0f,
            ...
            internalThresholdSlider.ToolTip = WirelessPowerStrings.InternalBatteryThresholdTitleTooltip;
            internalThresholdSlider.OnRealize += (PUIDelegates.OnRealize)(obj => this.InternalBatteryThresholdSlider = obj);
            internalThresholdSlider.OnValueChanged = ChangeInternalThreshold;
            internalThresholdSlider_components.AddChild(internalThresholdSlider);

            PLabel internalThresholdSliderMax_label = new PLabel("InternalThresholdSliderMaxLabel");
            internalThresholdSliderMax_label.Text = Mathf.RoundToInt(100).ToString();
            internalThresholdSliderMax_label.TextStyle = PUITuning.Fonts.TextDarkStyle;
            internalThresholdSlider_components.AddChild(internalThresholdSliderMax_label);

            internalThresholdSlider_components.AddTo(this.gameObject, -2);
        }

        private void ChangeTextFieldInternalThreshold(GameObject _, string threshold)
        {
            if (this.target == null)
                return;
            ...
            if (converted)
                this.ChangeInternalThreshold(null, newThreshold);
            else if (this.InternalBatteryThresholdText != null)
                PUIElements.SetText(this.InternalBatteryThresholdText, this.target.BatteryRefillPercent.ToString("0"));
        }

        private void ChangeInternalThreshold(GameObject _, float threshold)
        {
            if (this.target == null || !(this.target is WirelessPowerReceiver))
                return;

            int roundedThreshold = Mathf.RoundToInt(threshold);
            float newThreshold = this.target.BatteryRefillPercent = roundedThreshold / 100f;
            SetInternalThresholdUI(roundedThreshold);
        }

        private void SetInternalThresholdUI(float threshold)
        {
            if (this.target == null)
                return;

            PSliderSingle.SetCurrentValue(this.InternalBatteryThresholdSlider, threshold);

            if (this.WirelessBatteryThresholdText == null)
                return;
            PUIElements.SetText(this.InternalBatteryThresholdText, threshold.ToString("0"));
        }

        public override void SetTarget(GameObject target)
        {
            this.target = target.GetComponentSafe<WirelessPowerReceiver>();
            this.UpdateUI();
        }
        ...
        private void UpdateUI() 
        {
            ...
            this.SetInternalThresholdUI(this.target.BatteryRefillPercent * 100);
        }
    }
}