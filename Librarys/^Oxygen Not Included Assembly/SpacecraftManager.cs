...
public class SpacecraftManager : KMonoBehaviour, ISim1000ms
{
  ...
  {
    SpacecraftManager.instance = (SpacecraftManager) null;
  }
  ...
  private void GenerateRandomDestinations()
  {
    ...
    for (int index1 = 0; index1 < num2; ++index1)
    {
      ...
      int distance = intList[index2];
      intList.RemoveAt(index2);
      List<string> stringList = stringListList[distance];
      this.destinations.Add(new SpaceDestination(this.destinations.Count, stringList[random.Next(0, stringList.Count)], distance));
    }
  ...
  {
    this.gameObject.AddOrGet<Notifier>().Add(new Notification((string) BUILDING.STATUSITEMS.SPACECRAFTREADYTOLAND.NOTIFICATION, NotificationType.Good, HashedString.Invalid, (Func<List<Notification>, object, string>) ((notificationList, data) => (string) BUILDING.STATUSITEMS.SPACECRAFTREADYTOLAND.NOTIFICATION_TOOLTIP + notificationList.ReduceMessages(false)), (object) spacecraft.launchConditions.GetProperName(), false, 0.0f, (Notification.ClickCallback) null, (object) null, (Transform) null), string.Empty);
  }
  ...
  private void SpawnMissionResults(Dictionary<SimHashes, float> results)
  {
    foreach (KeyValuePair<SimHashes, float> result in results)
      ElementLoader.FindElementByHash(result.Key).substance.SpawnResource(PlayerController.GetCursorPos(KInputManager.GetMousePos()), result.Value, 300f, (byte) 0, 0, false, false, false);
  }
  ...
  public void EarnDestinationAnalysisPoints(int destinationID, float points)
  {
    ...
    SpacecraftManager.DestinationAnalysisState destinationAnalysisState1 = this.GetDestinationAnalysisState(destination);
    Dictionary<int, float> destinationAnalysisScores;
    int index;
    (destinationAnalysisScores = this.destinationAnalysisScores)[index = destinationID] = destinationAnalysisScores[index] + points;
    SpacecraftManager.DestinationAnalysisState destinationAnalysisState2 = this.GetDestinationAnalysisState(destination);
    if (destinationAnalysisState1 == destinationAnalysisState2)
      return;
