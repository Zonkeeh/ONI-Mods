...
public class GermExposureMonitor : GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance>
{
  ...
  public class Instance : GameStateMachine<GermExposureMonitor, GermExposureMonitor.Instance, IStateMachineTarget, object>.GameInstance
  {
    [Serialize]
    ...
    public override void StopSM(string reason)
    {
      ...
      foreach (ExposureType exposureType in GERM_EXPOSURE.TYPES)
      {
        ...
        guid = this.GetComponent<KSelectable>().RemoveStatusItem(guid, false);
      }
      base.StopSM(reason);
    }
      ...
      Sickness.InfectionVector vector)
    {
      foreach (ExposureType exposureType in GERM_EXPOSURE.TYPES)
      {
        ...
        {
          Sickness sickness = exposureType.sickness_id == null ? (Sickness) null : Db.Get().Sicknesses.Get(exposureType.sickness_id);
          if (sickness == null || sickness.infectionVectors.Contains(vector))
          {
            ...
            switch (exposureState)
            {
              ...
              case GermExposureMonitor.ExposureState.Contact:
                float contractionChance1 = GermExposureMonitor.GetContractionChance(this.GetResistanceToExposureType(exposureType, -1f));
                  ...
                  if ((double) contractionChance2 > 0.0)
                  {
                    ...
                    if (!exposureType.infect_immediately)
                    {
                      ...
                      if (flag1)
                      {
                        ...
                        float amount = Mathf.Clamp01(GermExposureMonitor.GetContractionChance(this.GetResistanceToExposureType(exposureType, -1f)) - contractionChance2);
                        GermExposureTracker.Instance.AddExposure(exposureType, amount);
                        continue;
                      }
                      continue;
    ...
    private bool HasMinExposurePeriodElapsed(string germ_id)
    {
      ...
      if ((double) num == 0.0)
        return true;
      return (double) (GameClock.Instance.GetTime() - num) > 540.0;
    }
    ...
    private void RefreshStatusItems()
    {
      foreach (ExposureType exposureType in GERM_EXPOSURE.TYPES)
      {
        ...
        if (guid2 == Guid.Empty && (exposureState == GermExposureMonitor.ExposureState.Exposed || exposureState == GermExposureMonitor.ExposureState.Contracted))
          ...
          guid2 = this.GetComponent<KSelectable>().RemoveStatusItem(guid2, false);
        ...
        if (guid1 == Guid.Empty && exposureState == GermExposureMonitor.ExposureState.Contact)
          ...
          guid1 = this.GetComponent<KSelectable>().RemoveStatusItem(guid1, false);
    ...
    {
      GermExposureMonitor.Instance.DiseaseSourceInfo diseaseSourceInfo;
      if (this.lastDiseaseSources.TryGetValue((HashedString) germ_id, out diseaseSourceInfo))
