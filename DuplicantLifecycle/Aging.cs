...
namespace DuplicantLifecycle
{
    ...
    public class Aging : StateMachineComponent<Aging.StatesInstance>
    {
        ...
        protected bool IsImmortal() => CurrentMinionAge() <= this.middleAge;
        ...
        public void RemoveModifiers(List<AttributeModifier> toRemove)
        {
            Attributes attributes = this.gameObject.GetAttributes();
        ...
        public class StatesInstance : GameStateMachine<Aging.States, Aging.StatesInstance, Aging, object>.GameInstance
        {
            ...
            {
                Death oldAge = new Death(
                    id:"Aging", 
                    parent:(ResourceSet) Db.Get().Deaths, 
                    name: "Old Age", 
                    description: "{Target} has sadly passed away due to their old age.", 
